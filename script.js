...
(function loadSupabaseAndInit() {
  if (typeof supabase !== "undefined" && supabase && supabase.createClient) {
    ...
    script.onerror = function () {
      console.error(
        "Failed to load Supabase library, running without Supabase",
      );
...
function initializeApp() {
  ...
  window.SUPABASE_ANON_KEY =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFrc2djc3dub2h2dnhubW1jcXNpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY2OTA2NDksImV4cCI6MjA5MjI2NjY0OX0.SPkDrNB71gGqVccM4j6m1MQKc5L_X1liYc_Fsp1a_AI";
...
function startFarmApp() {
  ...
  function initCharts() {
    ...
    const ctxKelembaban = document
      .getElementById("kelembabanChart")
      .getContext("2d");
    ...
    suhuChart = new Chart(ctxSuhu, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            backgroundColor: "rgba(26, 48, 38, 0.95)",
            titleColor: "#D4AF37",
            bodyColor: "#f6f8f7",
            borderColor: "#D4AF37",
            borderWidth: 1,
  ...
  function saveChartDataToLocal() {
    const chartData = {
      suhu: suhuDataPoints,
      kelembaban: kelembabanDataPoints,
      waktu: waktuDataPoints,
  ...
  function resetCharts() {
    ...
    if (umurFeedback) {
      ...
      setTimeout(() => {
        umurFeedback.textContent = originalText;
      }, 2000);
    }
  }
  ...
  function updateUmurHarian() {
    ...
    if (lastUpdate !== today) {
      ...
      umurFeedback.textContent =
        "Umur ayam bertambah! Sekarang " + umurAyam + " hari";
      umurFeedback.style.color = "#D4AF37";
  ...
  async function saveMode(mode) {
    ...
    try {
      await window.supabaseClient
        .from("system_config")
        .insert([{ mode: mode, updated_at: new Date().toISOString() }]);
    } catch (error) {
  ...
  function updateSensorDisplay() {
    if (suhuElement)
      suhuElement.textContent =
        useSimulatedData || (suhu === 0 && kelembaban === 0)
          ? "--°C"
          : Math.round(suhu) + "°C";
    if (kelembabanElement)
      kelembabanElement.textContent =
        useSimulatedData || (suhu === 0 && kelembaban === 0)
          ? "--%"
          : Math.round(kelembaban) + "%";
  }

  async function checkForNewSensorData() {
  ...
  function setUmurAyam() {
    ...
    if (umurAyam === 0) umurFeedback.textContent = "Ayam baru lahir!";
      ...
      umurFeedback.textContent =
        "Ayam siap panen! Umur sudah " + umurAyam + " hari";
  ...
  function switchMode(mode) {
    ...
    if (mode === "AUTO") {
      ...
      if (!useSimulatedData) autoControl();
    } else {
      toggles.forEach((toggle) => {
    ...
    modeBtns.forEach((btn) => {
      if (btn && btn.textContent.trim() === mode) btn.classList.add("active");
      ...
      newToggle.addEventListener("change", function (e) {
        if (currentMode === "MANUAL") {
          ...
          warning.style.cssText =
            "color:#D4AF37;font-size:11px;margin-top:5px;background:rgba(0,0,0,0.5);padding:2px 5px;border-radius:5px;";
  ...
  async function fetchSensorDataByDateRange(startDate, endDate) {
    if (!window.supabaseClient) return [];
    try {
      console.log(`🔍 FETCHING DATA: ${startDate} to ${endDate}`);
      ...
      console.log(`🟢 Start filter: ${startISO}`);
      console.log(`🔴 End filter: ${endISO}`);

      // AMBIL SEMUA DATA dengan pagination
      let allData = [];
      let page = 0;
      const pageSize = 1000; // Ambil 1000 per request
      let hasMore = true;
      ...
      while (hasMore) {
        ...
        if (error) {
          ...
          // Jika error karena range, coba tanpa range
          if (error.message && error.message.includes("range")) {
            console.log("🔄 Trying without range...");
            ...
            if (!fallbackError && fallbackData) {
              console.log(
                `📊 Found ${fallbackData.length} sensor data (fallback)`,
              );
              return fallbackData;
            }
          }
        ...
        if (!data || data.length === 0) {
          ...
          console.log(
            `  ✅ Got ${data.length} records, total: ${allData.length}`,
          );
      ...
      console.log(`📊 Total found: ${allData.length} sensor data`);

      if (allData.length > 0) {
        console.log(
          `  First: ${new Date(allData[0].created_at).toLocaleString("id-ID")}`,
        );
        console.log(
          `  Last: ${new Date(allData[allData.length - 1].created_at).toLocaleString("id-ID")}`,
        );
      }

      return allData;
    } catch (error) {
      console.error("❌ Error in fetchSensorDataByDateRange:", error);
      return [];
    }
  }

  async function getDataStatistics(startDate, endDate) {
    ...
    try {
      ...
      // Hitung total data dalam rentang
      ...
      const { data: latest, error: lError } = await window.supabaseClient
        .from("sensors")
        .select("created_at")
      ...
      if (!lError && latest && latest.length > 0) {
        lastDataDate = new Date(latest[0].created_at);
      }

      return {
        totalData: count || 0,
        lastDataDate: lastDataDate,
        lastDataDateStr: lastDataDate
          ? lastDataDate.toLocaleDateString("id-ID")
          : "Tidak ada data",
      };
    } catch (error) {
  ...
  async function fetchControlHistoryByDateRange(startDate, endDate) {
    ...
    try {
      ...
      console.log(`🔍 FETCHING CONTROL DATA: ${startDate} to ${endDate}`);
      ...
      while (hasMore) {
        ...
        const { data, error } = await window.supabaseClient
          .from("control_status")
          .select("control1, control2, control3, updated_at")
      ...
      console.log(`📊 Control records: ${allData.length}`);
      return allData;
    } catch (error) {
      console.error("❌ Error fetching control:", error);
      return [];
  ...
  async function fetchModeHistoryByDateRange(startDate, endDate) {
    ...
    try {
      ...
      console.log(`🔍 FETCHING MODE DATA: ${startDate} to ${endDate}`);
      ...
      console.log(`📊 Mode records: ${allData.length}`);
      return allData;
    } catch (error) {
  ...
  function aggregateDataByInterval(data, intervalMinutes) {
    ...
    if (intervalMinutes === 1)
      return data.map((d) => ({ ...d, type: "original" }));

    const aggregated = [];
    const intervalMs = intervalMinutes * 60 * 1000;

    // PASTIKAN data di-sort berdasarkan created_at
    const sortedData = [...data].sort(
      (a, b) =>
        new Date(a.created_at).getTime() - new Date(b.created_at).getTime(),
    );

    // Ambil timestamp pertama sebagai awal interval
    let currentIntervalStart = new Date(sortedData[0].created_at);
    // Reset ke menit ke-0
    currentIntervalStart.setMilliseconds(0);
    currentIntervalStart.setSeconds(0);
    // Bulatkan ke bawah ke interval terdekat
    const minutes = currentIntervalStart.getMinutes();
    const roundedMinutes =
      Math.floor(minutes / intervalMinutes) * intervalMinutes;
    currentIntervalStart.setMinutes(roundedMinutes);

    console.log(
      `📊 Starting aggregation from: ${currentIntervalStart.toLocaleString("id-ID")}`,
    );

    let sumSuhu = 0,
      sumKelembaban = 0,
      count = 0;
    let currentEndTime = new Date(currentIntervalStart.getTime() + intervalMs);

    for (const item of sortedData) {
      const itemDate = new Date(item.created_at);

      // Jika itemDate >= currentEndTime, maka pindah ke interval berikutnya
      if (itemDate.getTime() >= currentEndTime.getTime()) {
        // Simpan interval sebelumnya jika ada data
        if (count > 0) {
          aggregated.push({
            start_time: new Date(currentIntervalStart),
            end_time: new Date(currentEndTime),
            avg_suhu: sumSuhu / count,
            avg_kelembaban: sumKelembaban / count,
            count: count,
          });
        }

        // Pindah ke interval berikutnya
        currentIntervalStart = new Date(currentEndTime);
        currentEndTime = new Date(currentIntervalStart.getTime() + intervalMs);
        sumSuhu = 0;
        sumKelembaban = 0;
        count = 0;

        // Masukkan data ini ke interval baru
        sumSuhu += item.suhu;
        sumKelembaban += item.kelembaban;
        count = 1;
      } else {
        // Masukkan ke interval saat ini
        sumSuhu += item.suhu;
        sumKelembaban += item.kelembaban;
        count++;
      }
    ...
    console.log(
      `📊 Aggregated ${data.length} data into ${aggregated.length} intervals`,
    );
    return aggregated;
  }
  ...
  async function getAvailableDateRange() {
    ...
    try {
      // Cari data paling awal
      const { data: earliest, error: eError } = await window.supabaseClient
        .from("sensors")
        .select("created_at")
        .order("created_at", { ascending: true })
        .limit(1);

      // Cari data paling akhir
      const { data: latest, error: lError } = await window.supabaseClient
        .from("sensors")
        .select("created_at")
        .order("created_at", { ascending: false })
      ...
      const endDate = new Date(latest[0].created_at);
  ...
  function getControlStatusAtTime(controlHistory, targetTime) {
    ...
    if (!foundRecord && controlHistory.length > 0) {
      ...
      console.log(
        `[INFO] No control before ${new Date(targetTimestamp).toLocaleString()}, using earliest control: ${new Date(foundRecord.updated_at).toLocaleString()}`,
      );
    }
  ...
  function getModeAtTime(modeHistory, targetTime) {
    let latestMode = "AUTO";
    for (const record of modeHistory) {
      if (new Date(record.updated_at) <= targetTime) latestMode = record.mode;
      else break;
    }
    return latestMode;
  }

  function generateCSVWithFilters(
    sensorData,
    controlHistory,
    modeHistory,
    intervalMinutes,
    ...
    csvRows.push([
      "Status Lampu",
      getControlStatusAtTime(controlHistory, new Date()).control1
        ? "AKTIF"
        : "NONAKTIF",
    ]);
    csvRows.push([
      "Status Pompa",
      getControlStatusAtTime(controlHistory, new Date()).control2
        ? "AKTIF"
        : "NONAKTIF",
    ]);
    csvRows.push([
      "Status Kipas",
      getControlStatusAtTime(controlHistory, new Date()).control3
        ? "AKTIF"
        : "NONAKTIF",
    ]);
    csvRows.push([""]);
    ...
    if (intervalMinutes === 1) {
      ...
      for (const item of sensorData) {
        ...
        const control = getControlStatusAtTime(
          controlHistory,
        ...
        csvRows.push([
          ...
          item.kelembaban,
          mode,
          control.control1 ? "AKTIF" : "NONAKTIF",
          control.control2 ? "AKTIF" : "NONAKTIF",
          control.control3 ? "AKTIF" : "NONAKTIF",
        ]);
      }
    } else {
      ...
      for (const interval of aggregatedData) {
        const midTime = new Date(
          (interval.start_time.getTime() + interval.end_time.getTime()) / 2,
        );
        const mode = getModeAtTime(modeHistory, midTime);
        const control = getControlStatusAtTime(controlHistory, midTime);
        csvRows.push([
          interval.start_time.toLocaleString("id-ID"),
          interval.end_time.toLocaleString("id-ID"),
          interval.avg_suhu.toFixed(1),
          interval.avg_kelembaban.toFixed(1),
          interval.count,
          mode,
          control.control1 ? "AKTIF" : "NONAKTIF",
          control.control2 ? "AKTIF" : "NONAKTIF",
  ...
  async function downloadHistoryPDF() {
    ...
    if (!startDate || !endDate) {
      showDownloadFeedback("⚠️ Silakan pilih periode tanggal terlebih dahulu!");
      return;
    }

    // Validasi: startDate harus <= endDate
    if (new Date(startDate) > new Date(endDate)) {
      showDownloadFeedback(
        "⚠️ Tanggal mulai harus lebih awal dari tanggal akhir!",
      );
      return;
    }
    ...
    const displayData =
      interval === 1
        ? sensorData.slice(0, maxRows)
        : aggregatedData.slice(0, maxRows);
...
<body>
    ...
    if (printWindow) {
      ...
      showDownloadFeedback(
        "❌ Popup diblokir. Izinkan popup untuk download PDF.",
      );
    }
  }
  ...
  async function showDownloadModal() {
    ...
    if (modal) {
      ...
      // Ambil rentang data yang tersedia
      ...
      if (range) {
        ...
        console.log(`📅 Data available from ${range.start} to ${range.end}`);
        ...
        let lastDataInfo = "";
        if (stats && stats.lastDataDate) {
          const lastDate = new Date(stats.lastDataDate);
  ...
  async function downloadHistoryCSV() {
    ...
    showDownloadFeedback(
      `📡 Mengambil data dari ${startDate} s/d ${endDate}...`,
    );
    ...
    console.log(`📊 HASIL:`);
    console.log(`  Sensor: ${sensorData.length} data`);
    console.log(`  Control: ${controlHistory.length} records`);
    console.log(`  Mode: ${modeHistory.length} records`);

    if (sensorData.length > 0) {
      const firstDate = new Date(sensorData[0].created_at);
      const lastDate = new Date(sensorData[sensorData.length - 1].created_at);
      const daysActual =
        Math.ceil((lastDate - firstDate) / (1000 * 60 * 60 * 24)) + 1;
      const hoursActual = Math.ceil((lastDate - firstDate) / (1000 * 60 * 60));

      console.log(
        `  📅 Data aktual: ${firstDate.toLocaleString("id-ID")} - ${lastDate.toLocaleString("id-ID")}`,
      );
      console.log(`  📊 ${daysActual} hari, ${hoursActual} jam`);

      // Tampilkan feedback ke user
      let detailMessage = "";
      if (daysActual === 1) {
        detailMessage = ` (hanya 1 hari: ${firstDate.toLocaleDateString("id-ID")})`;
      } else {
        detailMessage = ` (${daysActual} hari: ${firstDate.toLocaleDateString("id-ID")} - ${lastDate.toLocaleDateString("id-ID")})`;
      }

      showDownloadFeedback(
        `📊 ${sensorData.length} data dari ${firstDate.toLocaleDateString("id-ID")} - ${lastDate.toLocaleDateString("id-ID")}${detailMessage}`,
      );
    }

    if (!sensorData || sensorData.length === 0) {
      showDownloadFeedback(
        `⚠️ Tidak ada data pada periode ${startDate} s/d ${endDate}!`,
      );
      return;
    }

    const csvData = generateCSVWithFilters(
      sensorData,
      controlHistory,
      modeHistory,
      interval,
    ...
    // Nama file dengan tanggal aktual
    let actualDateRange = "";
    if (sensorData.length > 0) {
      const firstDate = new Date(sensorData[0].created_at);
      const lastDate = new Date(sensorData[sensorData.length - 1].created_at);
      actualDateRange = `_${firstDate.toISOString().slice(0, 10)}_to_${lastDate.toISOString().slice(0, 10)}`;
    }

    const filename = `smart_control${actualDateRange}_${new Date().toISOString().slice(0, 19).replace(/:/g, "-")}.csv`;
    ...
    const daysActual =
      sensorData.length > 0
        ? Math.ceil(
            (new Date(sensorData[sensorData.length - 1].created_at) -
    ...
    showDownloadFeedback(
      `✅ CSV berhasil didownload! (${sensorData.length} data dari ${daysActual} hari)`,
    );
  }
  ...
  async function init() {
    ...
    switchMode(
      savedMode === "AUTO" || savedMode === "MANUAL" ? savedMode : "AUTO",
    );
    ...
    if (resetKelembabanBtn)
      resetKelembabanBtn.addEventListener("click", resetCharts);

    if (setUmurBtn) setUmurBtn.addEventListener("click", setUmurAyam);
    if (umurInput)
      umurInput.addEventListener("keypress", (e) => {
    ...
    console.log(
      "📥 Fitur Download: Pilih tanggal dan interval, lalu download CSV/PDF",
    );
  }
